...
const prevPage = () =>{
  ...
  {!currentJobs &&  <Loader/>}
  {currentJobs &&  <div className={scss.container}>
      <ul>
        {currentJobs.map(job => (
      ...
      <Pagination currentPage= {currentPage} jobsPerPage ={jobsPerPage} totalJobs={allJobs.length} paginate ={paginate}/>
      <button className={scss.prevButton} type='button' onClick={prevPage}> <IoIosArrowBack className={scss.iconArrowLeft}/></button>
      <button className={scss.nextButton} type='button' onClick={nextPage}> <IoIosArrowForward className={scss.iconArrowRigth}/></button>
      </div>
    </div>}
